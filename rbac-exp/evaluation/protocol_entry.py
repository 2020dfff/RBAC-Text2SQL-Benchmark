"""Command-line routing of the evaluator entry points to evaluate_protocol_v2."""
import sys
...
def dispatch_cli(crud=False):
    """Run the current evaluator; return only when --protocol_v1 asks for the v1 evaluator."""
    if "--protocol_v1" in sys.argv:
        sys.argv.remove("--protocol_v1")
        warnings.warn("protocol v1: SQL of answers to allowed requests is not checked against the policy",
                      RuntimeWarning)
        return
