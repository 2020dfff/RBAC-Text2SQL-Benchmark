...
def init_adapter(
    ...
    if finetuning_args.finetuning_type == "lora":
        ...
        if model_args.checkpoint_dir is not None:
            # Check for either .bin or .safetensors adapter weights
            has_bin_weights = os.path.exists(
                os.path.join(model_args.checkpoint_dir[0], WEIGHTS_NAME)
            )
            has_safetensors_weights = os.path.exists(
                os.path.join(model_args.checkpoint_dir[0], SAFETENSORS_WEIGHTS_NAME)
            )
            assert has_bin_weights or has_safetensors_weights, \
                "Provided path ({}) does not contain a LoRA weight (adapter_model.bin or adapter_model.safetensors).".format(
