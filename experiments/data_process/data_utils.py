...
def extract_sql_role_prompt_dataset(example: Dict[str, Any]) -> Dict[str, str]:
    """
    Extract and construct clean role-based prompt from potentially messy data format.
    Handles cases where instruction and input fields may contain formatting artifacts.
    """
    import re
    
    # Clean the instruction field to extract pure schema information
    instruction = example.get("instruction", "")
    if "##Instruction:" in instruction:
        # Extract content after ##Instruction:
        idx = instruction.find("##Instruction:")
        clean_instruction = instruction[idx + len("##Instruction:"):].strip()
    else:
        clean_instruction = instruction
    
    # Clean the input field to extract pure question text
    input_text = example.get("input", "")
    if "###Input:" in input_text and "###Response:" in input_text:
        # Extract content between ###Input: and ###Response:
        start = input_text.find("###Input:") + len("###Input:")
        end = input_text.find("###Response:")
        clean_input = input_text[start:end].strip()
    else:
        clean_input = input_text.strip()
    
    # Create clean example with extracted content
    clean_example = {
    ...
    # Use the cleaned data to format the prompt
    if clean_example.get("input", "") != "":
        prompt_format = SQL_ROLE_PROMPT_DICT["prompt_input"]
    else:
        prompt_format = SQL_ROLE_PROMPT_DICT["prompt_no_input"]
    
    return {"input": prompt_format.format(**clean_example)}
...
r"""
Supports: https://huggingface.co/google/gemma-3-4b-it
          https://huggingface.co/google/gemma-2-4b-it
          https://huggingface.co/google/gemma-2-2b-it
          https://huggingface.co/google/gemma-2-9b-it
Gemma models use a special conversation format with control tokens.
...
register_template(
    ...
    system="",  # Gemma doesn't use explicit system prompts in the same way
    sep=["\n"],
