...
def normalise_spider_difficulty(value: Optional[str]) -> str:
    diff = (value or "easy").strip().lower()
    if diff not in {"easy", "medium", "hard", "extra"}:
        return "extra"
    return diff


def load_spider_difficulties(dataset_path: Optional[str]) -> List[str]:
    if not dataset_path:
        return []
    ...
    if isinstance(payload, list):
        for item in payload:
            if isinstance(item, dict):
                difficulties.append(normalise_spider_difficulty(item.get("difficulty")))
    return difficulties


def has_schema_file(db_id, db_dir):
    """
    Check if a database has a schema.sql file.
    
    Args:
        db_id: Database identifier
        db_dir: Directory containing all databases
    
    Returns:
        bool: True if schema.sql exists, False otherwise
    """
...
def print_formated_s(row_name, l, element_format):
    ...
    line = template.format(row_name, *l)
    print(line)
    if hasattr(print_formated_s, "result_buffer") and print_formated_s.result_buffer is not None:
        print_formated_s.result_buffer.write(line + "\n")
...
def print_scores(scores, etype, include_turn_acc=True):
    ...
    print_formated_s.result_buffer = result_buffer
    ...
    if etype in ["all", "exec"]:
        ...
        result_buffer.write(f"compare etype {etype}\n")
        print("=====================   EXECUTION ACCURACY     =====================")
        result_buffer.write("=====================   EXECUTION ACCURACY     =====================\n")
        exec_scores = [f"{x:.3f}" for x in [scores[level]["exec"] for level in levels]]
        print_formated_s("execution", exec_scores, "{:20}")

    if etype in ["all", "match"]:
        print("\n====================== EXACT MATCHING ACCURACY =====================")
        result_buffer.write("\n====================== EXACT MATCHING ACCURACY =====================\n")
        exact_scores = [f"{x:.3f}" for x in [scores[level]["exact"] for level in levels]]
        print_formated_s("exact match", exact_scores, "{:20}")
        print("\n---------------------PARTIAL MATCHING ACCURACY----------------------")
        result_buffer.write("\n---------------------PARTIAL MATCHING ACCURACY----------------------\n")
        for type_ in partial_types:
            this_scores = [f"{x:.3f}" for x in [scores[level]["partial"][type_]["acc"] for level in levels]]
            print_formated_s(type_, this_scores, "{:20}")

        print("---------------------- PARTIAL MATCHING RECALL ----------------------")
        result_buffer.write("---------------------- PARTIAL MATCHING RECALL ----------------------\n")
        for type_ in partial_types:
            this_scores = [f"{x:.3f}" for x in [scores[level]["partial"][type_]["rec"] for level in levels]]
            print_formated_s(type_, this_scores, "{:20}")

        print("---------------------- PARTIAL MATCHING F1 --------------------------")
        result_buffer.write("---------------------- PARTIAL MATCHING F1 --------------------------\n")
        for type_ in partial_types:
            this_scores = [f"{x:.3f}" for x in [scores[level]["partial"][type_]["f1"] for level in levels]]
            print_formated_s(type_, this_scores, "{:20}")
    ...
    if include_turn_acc:
        ...
        if etype in ["all", "exec"]:
            ...
            result_buffer.write("=====================   TURN EXECUTION ACCURACY     =====================\n")
            exec_scores = [f"{x:.3f}" for x in [scores[turn]["exec"] for turn in turns]]
            print_formated_s("execution", exec_scores, "{:20}")

        if etype in ["all", "match"]:
            print("\n====================== TURN EXACT MATCHING ACCURACY =====================")
            result_buffer.write("\n====================== TURN EXACT MATCHING ACCURACY =====================\n")
            exact_scores = [f"{x:.3f}" for x in [scores[turn]["exact"] for turn in turns]]
            print_formated_s("exact match", exact_scores, "{:20}")

    input_filename = getattr(print_scores, "input_filename", None)
    fair_comparison = getattr(print_scores, "fair_comparison", False)
    if input_filename:
        base = re.sub(r"\.sql$|\.jsonl$|\.txt$", "", os.path.basename(input_filename))
        
        # Create eval_result directory in the same parent directory as input file
        input_dir = os.path.dirname(input_filename)
        eval_result_dir = os.path.join(os.path.dirname(input_dir), "eval_result")
        os.makedirs(eval_result_dir, exist_ok=True)
        
        if fair_comparison:
            result_filename = f"{base}_fair_comparison_evaluate_result.txt"
        else:
            result_filename = f"{base}_evaluate_result.txt"
        result_filename = os.path.join(eval_result_dir, result_filename)
    else:
        result_filename = "evaluate_result.txt"
    with open(result_filename, "w", encoding="utf-8") as f:
        f.write(result_buffer.getvalue())
    print_formated_s.result_buffer = None
...
def evaluate(
    ...
    base = re.sub(r"\.sql$|\.jsonl$|\.txt$", "", os.path.basename(predict))
    
    # Create eval_result directory in the same parent directory as predict file
    predict_dir = os.path.dirname(predict)
    eval_result_dir = os.path.join(os.path.dirname(predict_dir), "eval_result")
    os.makedirs(eval_result_dir, exist_ok=True)
    
    if fair_comparison:
        incorrect_filename = f"{base}_fair_comparison_incorrect.txt"
    else:
        incorrect_filename = f"{base}_incorrect.txt"
    incorrect_log_file = open(os.path.join(eval_result_dir, incorrect_filename), "w")

    # Read gold and predict files simultaneously to maintain synchronization
    with open(gold) as gold_file, open(predict) as pred_file:
        glist = []
        plist = []
        gseq_one = []
        pseq_one = []
        ...
        pred_lines = pred_file.readlines() if predict.endswith(".sql") else None
        
        if predict.endswith(".jsonl"):
            pred_objs = [json.loads(l) for l in pred_file.readlines()]
        else:
            pred_objs = None
        
        pred_idx = 0  # Track prediction line index
        
        for gold_idx, gold_line in enumerate(gold_lines):
            if len(gold_line.strip()) == 0:
                gseq_one.append(["no out"])
                if pred_idx < len(pred_lines) if pred_lines else len(pred_objs):
                    pseq_one.append(["no out"])
                    pred_idx += 1
            else:
                gold_parts = gold_line.strip().split("\t")
                
                # If fair_comparison is enabled, check if database has schema file
                if fair_comparison and len(gold_parts) >= 2:
                    db_id = gold_parts[1]
                    if not has_schema_file(db_id, db_dir):
                        pred_idx += 1  # Skip corresponding prediction line
                        continue  # Skip this query if database has no schema file
                
                gseq_one.append(gold_parts)
                
                # Add corresponding prediction
                if pred_idx < (len(pred_lines) if pred_lines else len(pred_objs)):
                    if predict.endswith(".sql"):
                        ...
                        if len(pred_line.strip()) == 0:
                            ...
                            pseq_one.append(pred_line.strip().split("\t"))
                    elif predict.endswith(".jsonl"):
                        obj = pred_objs[pred_idx]
    ...
    difficulty_list = difficulties or []
    sample_idx = 0
    ...
    for i, (p, g) in enumerate(zip(plist, glist)):
        ...
        for idx, pg in enumerate(zip(p, g)):
            ...
            if sample_idx < len(difficulty_list):
                hardness = normalise_spider_difficulty(difficulty_list[sample_idx])
            else:
                hardness = evaluator.eval_hardness(g_sql)
                hardness = normalise_spider_difficulty(hardness)
            if idx > 3:
                idx = "> 4"
            else:
                idx += 1
    ...
    # print(scores)
    # 传递输入文件名给print_scores用于命名结果文件
    print_scores.input_filename = predict
    print_scores.fair_comparison = fair_comparison
...
def evaluate_api(args: Optional[Dict[str, Any]] = None):
    ...
    difficulty_list = load_spider_difficulties(args.get("difficulty_json", ""))
    ...
    evaluate(
        ...
        args.get("fair_comparison", False),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="whether to filter out databases without schema files for fair comparison with role-based evaluation",
    )
    parser.add_argument(
        "--difficulty_json",
        dest="difficulty_json",
        type=str,
        default="./data/selected/spider/spider_dev.json",
        help="Path to Spider JSON file containing gold difficulty annotations",
    )
    ...
    if args.natsql:
        ...
        args.gold = "./data/spider/dev_gold.sql"
        args.table = "./data/spider/tables.json"
    else:
    ...
    difficulty_list = load_spider_difficulties(getattr(args, "difficulty_json", ""))
